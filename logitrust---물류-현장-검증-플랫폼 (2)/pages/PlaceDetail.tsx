...
import { MapPin, Edit2, Star, Plus, Info, MessageSquare, Shield, History, ArrowRight, Camera, X, Map as MapIcon, Megaphone, Users, CheckCircle } from 'lucide-react';
...
export const PlaceDetail: React.FC = () => {
  ...
  const { places, getPlaceScore, getPlaceConstraints, getConstraintStatus, getPlaceVersions, reviews, currentUser, submitEditRequest, addReview, announcements } = useAppStore();
  ...
  const placeAnnouncements = useMemo(() => announcements.filter(a => a.placeId === id && a.isActive), [id, announcements]);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="px-8 pb-8 pt-0 relative">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 -mt-10">
            ...
            <div className="flex flex-col gap-3 items-start md:items-end">
               ...
               {/* Metadata Badges */}
               <div className="flex gap-2">
                   <span className="px-2 py-1 rounded bg-slate-100 text-[10px] font-bold text-slate-500 flex items-center gap-1">
               ...
               {currentUser?.role === Role.DRIVER && (
                  <button 
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold shadow-md shadow-blue-200 transition-all hover:shadow-lg transform hover:-translate-y-0.5 mt-2"
                  >
      ...
      {placeAnnouncements.length > 0 && (
         <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 shadow-sm animate-in fade-in slide-in-from-top-2">
            <div className="flex items-start gap-4">
               <div className="p-2 bg-blue-100 rounded-lg text-blue-600 flex-shrink-0">
                  <Megaphone className="w-5 h-5" />
               </div>
               <div>
                  <h3 className="font-bold text-blue-900 text-sm mb-1">{placeAnnouncements[0].title}</h3>
                  <p className="text-sm text-blue-800 leading-relaxed">{placeAnnouncements[0].content}</p>
                  <p className="text-xs text-blue-400 mt-2 font-medium">{placeAnnouncements[0].createdAt.split('T')[0]} 등록됨</p>
               </div>
            </div>
         </div>
      )}

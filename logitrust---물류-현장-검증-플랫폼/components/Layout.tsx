...
import { Truck, ShieldCheck, LogOut, MapPin, Bell } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="hidden sm:flex sm:space-x-4">
                {currentUser && (
                  <Link 
                    to="/places" 
                    className={`px-3 py-2 rounded-md text-sm transition-colors ${location.pathname.startsWith('/places') ? 'bg-slate-100 text-slate-900 font-semibold' : 'text-slate-500 hover:text-slate-900'}`}
                  >
                    장소 목록
                  </Link>
                )}
                ...
                <div className="flex items-center gap-4">
                   ...
                   <div className="flex flex-col items-end hidden sm:flex">
                    ...
                    <span className="text-[10px] tracking-wider font-bold text-slate-500 uppercase bg-slate-100 px-1.5 py-0.5 rounded">
                      {currentUser.role === Role.DRIVER ? 'DRIVER' : currentUser.role === Role.OPS ? 'OPS TEAM' : 'MANAGER'}
                    </span>
                   </div>
                   <button 
      ...
      {currentUser && (
        <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-200 flex justify-around py-3 z-50 pb-safe">
          <Link to="/places" className={`flex flex-col items-center p-2 rounded-lg ${location.pathname.startsWith('/places') ? 'text-blue-600' : 'text-slate-400'}`}>
